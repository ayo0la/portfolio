...
date: 2026-08-10
tldr: "Our agents drafted messages that sounded like an AI. The fix was not a better prompt or a fine-tune, it was noticing that every edit staff already made was free film we were throwing away. Shipped this week."
---
...
Every serious coach watches the film. We had a room full of film on how each of these directors plays, and we had never once pressed play.
...
That edit is a training signal. It is better than a training signal, actually: it is free, it is unambiguous, and it is the one thing no prompt of mine was ever going to guess. The staff member is not annotating data for us. She is doing her job, and between what the machine wrote and what she sent, in that thin gap, sits the entire lesson.
...
Architecture is the easy part, and the part that photographs well. These are the choices I would want to read about in someone else's post.
...
The approve form is a textarea prefilled with the draft. Browsers normalize line breaks in textarea submissions to carriage-return-newline, while our stored draft used bare newlines. So a director who read a three-paragraph draft, changed nothing, and hit approve produced a "final" text that differed from the draft by invisible bytes on every line break. Byte comparison said edit. Offside by a shoelace, and the goal comes off the board. The distiller then dutifully learned style lessons from a diff containing no style changes.

The fix is one function that normalizes both sides before comparing. Two lines, after all that. But what do these systems do when the input is subtly wrong? They do not crash. They do not complain. They confidently learn garbage. That is why the whole thing was built test-first, and why the tests cover the boring cases like "approval with Windows line endings" rather than only the happy path.
...
Which means the interesting part starts now. Nothing in the paragraphs above is proven yet. A profile that reads well after five gate actions may say something strange after two hundred, and the only way to find out is to watch real directors approve real messages for a few weeks and read what the system decided they sound like. That is the actual test, and I do not get to grade it myself. Time will tell.

The general lesson is the part that travels. If your product puts a human in front of AI output, you are already collecting the most valuable data in the building, and you are probably discarding it on every single approval. The film has been running the whole time, in every queue, on every approval. The gap between what the model wrote and what the human sent is your product's opinion about itself. Store it, distill it, feed it back.
