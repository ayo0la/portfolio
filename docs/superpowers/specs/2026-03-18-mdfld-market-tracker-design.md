...
**Slug generation rule:** Slugs are generated by `normalise.ts`, not by individual site scrapers. Format: `{brand}-{name}-{colorway}` lowercased, spaces replaced with hyphens, special characters stripped. Example: `nike-mercurial-superfly-ix-elite-blackout`. If colorway is null, omit it. This ensures that two scrapers independently scraping the same boot produce the same slug and hit the same `products` row.
...
listing_id_external text            -- ID on source site (for deduplication)
...
UNIQUE(site, listing_id_external) WHERE listing_id_external IS NOT NULL
UNIQUE(site, url)                  -- fallback dedup when listing_id_external is null
```

**Deduplication rule:** `db.ts` uses upsert-on-conflict. On every scrape cycle, each listing is upserted: if `(site, listing_id_external)` already exists the row is updated (price, in_stock, scraped_at, last_price); if `listing_id_external` is null, conflict is detected on `(site, url)`. New listings are inserted. Listings not seen in the last 24 hours are marked `in_stock = false`.
...
price            numeric NOT NULL
currency         text NOT NULL DEFAULT 'GBP'
price_gbp        numeric          -- normalised to GBP (written by normalise.ts)
price_change_pct numeric          -- % change from previous record for this product+site
size_uk          text             -- size context for the recorded price
recorded_at      timestamptz DEFAULT now()
```

**Price history write rule:** After each scrape cycle, `db.ts` records one row per `(product_id, site)` — the **lowest-priced in-stock listing** for that product on that site at that moment. This keeps the chart clean and comparable across sites. Size is recorded alongside the price for context but the chart plots all sizes together by default (size filter on the product page filters the listings table, not the chart).
...
value            jsonb NOT NULL  -- shape depends on metric (see below)
volume_7d        numeric         -- only populated for metric = 'volume'
price_change_7d  numeric         -- only populated for metric = 'avg_price'
trending_score   numeric         -- only populated for metric = 'trending'
recorded_at      timestamptz DEFAULT now()
```

**Row shapes per metric:**
```jsonc
// metric = 'avg_price'
{ "price": 312.50 }

// metric = 'volume'  (number of active listings in category)
{ "count": 4821 }

// metric = 'top_mover'  (one row per top-moving product)
{ "product_id": "uuid", "slug": "nike-mercurial-superfly-ix", "name": "Mercurial Superfly IX", "change_pct": 14.2, "avg_price": 412 }

// metric = 'trending'  (one row per trending product, ordered by trending_score)
{ "product_id": "uuid", "slug": "puma-king-platinum-fg", "name": "Puma King Platinum FG", "lowest_price": 195, "site_count": 5 }
```

The `GET /api/market` route returns the latest snapshot row for each distinct `metric` value across all categories.
...
scraper/
  ...
  normalise.ts      — slug generation, currency conversion, size normalisation (UK/EU/US),
...
**Currency normalisation rule:** `listings.price` is always stored in the original scraped currency. `price_history.price_gbp` is the normalised value, computed by `normalise.ts` at write time using a static daily GBP exchange rate stored in a `exchange_rates` table (one row per currency, updated once daily by a separate lightweight job). For v1, rates for EUR and USD are sufficient; other currencies default to GBP. `listings` has no `price_gbp` column — normalisation only applies to the history record.
...
- Both pages render the same `BrowsePage` component with a `category` prop — they are not separately implemented.
